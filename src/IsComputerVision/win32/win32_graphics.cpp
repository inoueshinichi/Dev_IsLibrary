...
namespace is
{
    namespace cv
    {
        ...
        void WinBmp::setup(int32_t width, int32_t height, int32_t channels)
        {
            ...
            else 
            {
                throw std::runtime_error(
                    common::format_string("Channels is not match. Given is %d", 
                                            channels));
            }
            ...
            bmi_info_->bmiHeader.biSizeImage = (DWORD)datasize_;
        ...
        BOOL WinBmp::get_data(BYTE* data, int extract_color)
        {
            if (!bmp_ || !bmi_info_)
                return false;
        ...
        BOOL WinBmp::save(const CString &filename, BOOL is_dump)
        {
            ...
            if (!fp)
            {
                std::cerr << common::format_string("Can not open %s", filename.GetString()) << std::endl;
            }
        ...
        BOOL WinBmp::load(const CString &filename, int &width, int &height, int &channels, BOOL is_dump)
        {
            ...
            try
            {
                ...
                /*BITMAPINFO*/
                size_t bmp_infosize = bmp_file_header_.bfOffBits - sizeof(BITMAPFILEHEADER);
                bmi_info_ = (BITMAPINFO *)new byte[bmp_infosize];
                fread(bmi_info_, bmp_infosize, 1, fp.get());
                this->width_ = bmi_info_->bmiHeader.biWidth;
                this->height_ = bmi_info_->bmiHeader.biHeight
                                    ? bmi_info_->bmiHeader.biHeight
                                    : -bmi_info_->bmiHeader.biHeight;

                // 4バイト境界単位の画像1行のメモリサイズを計算
                int padding = 0;
                uint16_t bit_count = bmi_info_->bmiHeader.biBitCount;
                size_t mem_width = 0;
                if (bit_count == 8)
                {
                    this->channels_ = 1;
                ...
                {
                    throw std::runtime_error(
                        common::format_string("Bitcount is not match. Given is %d",
                                                bit_count));
                }
