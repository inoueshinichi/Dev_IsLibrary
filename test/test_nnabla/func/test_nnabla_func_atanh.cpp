/**
 * @file test_nnabla_func_atanh.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla atanh-func
 * @version 0.1
 * @date 2021-12-17
 * 
...
namespace 
{
    TEST(nnabla_func, atanh_1)
    {
        ...
        data[2 * st[0] + 2 * st[1]] = 1;

        show_ndarray_contents<float>(ndarray_zeros);

        cout << "---" <<endl;

        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(atanh<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }


    TEST(nnabla_func, atanh_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
