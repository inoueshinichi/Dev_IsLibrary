/**
 * @file test_nnabla_func_sqrt.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla sqrt-func
 * @version 0.1
 * @date 2022-02-06
 * 
...
namespace 
{
    TEST(nnabla_func, sqrt_1)
    {
        ...
        data[2 * st[0] + 2 * st[1]] = 2.0;

        show_ndarray_contents<float>(ndarray_zeros);

        cout << "---" <<endl;

        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(sqrt<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, sqrt_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
