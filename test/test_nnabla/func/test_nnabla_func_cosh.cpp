/**
 * @file test_nnabla_func_cosh.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla cosh-func
 * @version 0.1
 * @date 2021-12-17
 * 
...
namespace 
{
    TEST(nnabla_func, cosh_1)
    {
        ...
        data[1 * st[0] + 1 * st[1]] = M_PI / 3;
        data[2 * st[0] + 2 * st[1]] = M_PI / 2;

        show_ndarray_contents<float>(ndarray_zeros);

        cout << "---" <<endl;

        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(cosh<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }


    TEST(nnabla_func, cosh_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        cout << "cos(M_PI/2)=" << cos(float(M_PI/2)) << endl;
    }
}
