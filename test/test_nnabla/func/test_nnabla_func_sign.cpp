/**
 * @file test_nnabla_func_sign.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla sign-func
 * @version 0.1
 * @date 2022-02-06
 * 
...
namespace 
{
    TEST(nnabla_func, sign_1)
    {
        ...
        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(sign<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, sign_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        for (int y = 0; y < sh[0]; ++y)
        {
            for (int x = 0; x < sh[1]; ++x)
            {
                ...
                v = (v > (float)0) ? (float)1 : ((v < (float)0) ? (float)-1 : (float)0);
            }
        }
