/**
 * @file test_nnabla_func_floor.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla floor-func
 * @version 0.1
 * @date 2022-02-06
 * 
...
namespace 
{
    TEST(nnabla_func, floor_1)
    {
        ...
        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(floor<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, floor_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
