/**
 * @file test_nnabla_func_r_pow_scalar.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla r_pow_scalar-func
 * @version 0.1
 * @date 2021-12-12
 * 
...
namespace 
{
    TEST(nnabla_func, r_pow_scalar_1)
    {
        using namespace std;
        ...
        auto out_ndarray = r_pow_scalar<float>(ndarray_adds, 2);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, r_pow_scalar_2)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
    ...
    TEST(nnabla_func, r_div_scalar_3)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        auto out_ndarray = r_pow_scalar<float>(ndarray_adds, 5);
