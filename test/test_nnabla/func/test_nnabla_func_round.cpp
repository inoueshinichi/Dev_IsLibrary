/**
 * @file test_nnabla_func_round.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla round-func
 * @version 0.1
 * @date 2022-02-06
 * 
...
namespace 
{
    TEST(nnabla_func, round_1)
    {
        ...
        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(round<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, round_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
