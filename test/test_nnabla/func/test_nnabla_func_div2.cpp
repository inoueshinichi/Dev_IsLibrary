/**
 * @file test_nnabla_func_div2.cpp
...
namespace 
{
    TEST(nnabla_func, div2_1)
    {
        ...
        auto output = div2<float>(in_narr1, in_narr2);
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }

    TEST(nnabla_func, div2_2)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();

        // input
        auto in_narr1 = ones<float>(Shape_t{3, 16, 16});
        in_narr1 = mul_scalar<float>(in_narr1, 3); // * 3
        auto st1 = in_narr1->strides();
        auto sh1 = in_narr1->shape();
        cout << "--- in_arr1" << endl;
        show_ndarray_contents<float>(in_narr1);

        auto in_narr2 = ones<float>(Shape_t{3, 16, 16});
        in_narr2 = mul_scalar<float>(in_narr2, 2); // * 1.5
        auto st2 = in_narr2->strides();
        auto sh2 = in_narr2->shape();
        cout << "--- in_arr2" << endl;
        show_ndarray_contents<float>(in_narr2);
    ...
    TEST(nnabla_func, div2_3)
    {
        ...
        auto in_narr2 = zeros<float>(Shape_t{3, 3, 1});
        auto st2 = in_narr2->strides();
        auto sh2 = in_narr2->shape();
        cout << "--- in_arr2" << endl;
        show_ndarray_contents<float>(in_narr2);

        auto output = div2<float>(in_narr1, in_narr2); // div by zero error
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }

    TEST(nnabla_func, native_div2)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        in_narr2 = mul_scalar<float>(in_narr2, 2); // * 2
        auto st2 = in_narr2->strides();
        auto sh2 = in_narr2->shape();
    ...
    TEST(nnabla_func, lowcode_div2)
    {
        ...
        for (int c = 0; c < channels; ++c)
            for (int y = 0; y < rows; ++y)
                ...
                {
                    out[c * stride_c + y * stride_y + x * stride_x] = 
                        data1[c * stride_c + y * stride_y + x * stride_x] /
                        data2[c * stride_c + y * stride_y + x * stride_x];
                }
