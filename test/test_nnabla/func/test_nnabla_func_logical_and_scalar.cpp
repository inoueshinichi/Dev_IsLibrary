/**
 * @file test_nnabla_func_logical_and_scalar.cpp
...
namespace 
{
    TEST(nnabla_func, logical_and_scalar_1)
    {
        using namespace std;
        ...
        auto output = is::nbla::logical_and_scalar<float>(in_narr1, true);
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }

       TEST(nnabla_func, logical_and_scalar_2)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        auto output = is::nbla::logical_and_scalar<float>(in_narr1, false);
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }
}
