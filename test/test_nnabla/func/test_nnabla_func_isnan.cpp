/**
 * @file test_nnabla_func_isnan.cpp
...
namespace
{
     TEST(nnabla_func, isnan_1)
    {
        ...
        data[1 * st[0] + 1 * st[1]] = std::numeric_limits<float>::quiet_NaN();
        data[2 * st[0] + 2 * st[1]] = std::numeric_limits<float>::signaling_NaN();

        show_ndarray_contents<float>(ndarray_zeros);

        cout << "---" <<endl;

        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(is::nbla::isnan<float>, ndarray_zeros);
