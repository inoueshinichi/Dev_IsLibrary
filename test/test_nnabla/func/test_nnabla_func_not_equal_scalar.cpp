/**
 * @file test_nnabla_func_not_equal_scalar.cpp
...
namespace 
{
    TEST(nnabla_func, not_equal_scalar_1)
    {
        using namespace std;
        ...
        auto output = is::nbla::not_equal_scalar<float>(in_narr1, 5);
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }

    TEST(nnabla_func, not_equal_scalar_2)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        auto output = is::nbla::not_equal_scalar<float>(in_narr1, std::numeric_limits<float>::quiet_NaN());
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }

    TEST(nnabla_func, not_equal_scalar_3)
    {
        using namespace std;
        const auto &ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
        ...
        auto output = is::nbla::not_equal_scalar<float>(in_narr1, std::numeric_limits<float>::infinity());
        cout << "--- output" << endl;
        show_ndarray_contents<float>(output);
    }
}
