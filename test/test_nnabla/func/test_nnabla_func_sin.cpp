/**
 * @file test_nnabla_func_sin.cpp
 * @author your name (you@domain.com)
 * @brief [Test] Nnabla sin-func
 * @version 0.1
 * @date 2021-12-14
 * 
...
namespace 
{
    TEST(nnabla_func, sin_1)
    {
        ...
        auto out_ndarray = is::common::invoke_tm_chrono_ms_ret(sin<float>, ndarray_zeros);

        show_ndarray_contents<float>(out_ndarray);
    }

    TEST(nnabla_func, sin_2)
    {
        using namespace std;
        const auto& ctx_cpu = SingletonManager::get<GlobalContext>()->get_current_context();
